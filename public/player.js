...
try{S.server=localStorage.getItem('tangsky_server')||'2';}catch(e){}
try{S.playbackRate=parseFloat(localStorage.getItem('tangsky_playback_rate'))||1.0;}catch(e){S.playbackRate=1.0;}
...
function updateOG(title,image){
    var t=document.querySelector('meta[property="og:title"]');if(!t){t=document.createElement('meta');t.setAttribute('property','og:title');document.head.appendChild(t);}t.setAttribute('content',title+' | TangskyMusify');
    ...
    document.title=title+' - TangskyMusify';
}
...
function loadTrack(track,resumeAt){
    if(!track)return;
    // Handle offline playback dari IndexedDB
    if(track.ytUrl==='offline'){
        var offlineVideoId=track.videoId||track.id;
        if(typeof dlGet==='function'){
            dlGet(offlineVideoId).then(function(s){
                if(!s||!s.audioBlob){showToast('⚠️ Audio offline tidak ditemukan');return;}
                ...
                AU.src=url;AU.load();
                if(resumeAt)AU.currentTime=resumeAt;
                AU.play().catch(function(e){showToast('⚠️ Gagal putar offline: '+e.message);});
                S.server='2';S.ip=true;S.il=false;
                if(typeof UU==='function')UU();
                if(typeof UB==='function')UB();
            }).catch(function(){showToast('⚠️ Gagal membaca library offline');});
        }
        return;
    }
    ST();
        ...
        if(S.yp&&S.yr&&track.videoId){
            ...
            ytFallbackTimeout=setTimeout(function(){
                if(S.ct===track && !S.ip){
                    ...
                    S.server='2';
                    try{localStorage.setItem('tangsky_server','2');}catch(e){}
...
function setServer(v){
    ...
    S.server=v;
    try{localStorage.setItem('tangsky_server',v);}catch(e){}
...
// PLAYLIST SYSTEM
function getUserPlaylists(){try{return JSON.parse(localStorage.getItem('tangsky_playlists')||'[]');}catch(e){return[];}}
function saveUserPlaylists(pls){try{localStorage.setItem('tangsky_playlists',JSON.stringify(pls));}catch(e){}}
...
function setPlaybackSpeed(speed) {
    ...
    try {
        localStorage.setItem('tangsky_playback_rate', speed);
    } catch(e) {}
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                ...
                '<span class="text-[10px] text-[#6b7280] tracking-wider font-semibold uppercase">TangskyMusify Web App</span>' +
            '</div>' +
        '</div>' +
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: '🎵 Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di TangskyMusify!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
