...
window.addEventListener('appinstalled',function(){
    ...
    if(typeof showToast==='function')showToast('✅ TangskyMusify berhasil diinstall!');
});
function installPWA(){
    if(deferredInstallPrompt){
        deferredInstallPrompt.prompt();
        deferredInstallPrompt.userChoice.then(function(choice){
            if(choice.outcome==='accepted'&&typeof showToast==='function')showToast('✅ Menginstall TangskyMusify...');
            deferredInstallPrompt=null;
            var btn=document.getElementById('pwa-install-btn');
            if(btn)btn.classList.add('hidden');
        });
        ...
        if(typeof showToast==='function')showToast('⚠️ Aplikasi sudah terinstall atau tidak didukung browser ini');
    }
}
...
const App={
    init(){
        ...
        <div class="nav-blur pb-safe h-[65px] flex items-center justify-around fixed bottom-0 w-full z-40">
            ...
            <button onclick="App.switch('downloads')" id="nav-downloads" class="relative flex flex-col items-center text-[#6b7280] active:scale-90"><i data-lucide="download" class="w-5 h-5"></i><span class="text-[10px]">Unduhan</span></button>
        ...
        <div class="pt-12 px-4 text-center">
            ...
            <h1 class="text-3xl font-black chrome-text mb-1">TangskyMusify</h1>
            ...
            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-3 text-left mb-6">
                ...
                <div class="flex justify-between"><span class="text-[#6b7280] text-sm">Nama</span><span class="text-white font-medium text-sm">TangskyMusify</span></div>
            ...
            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-3 text-left mb-6">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-white font-medium text-sm">Tangsky</span>
                    </div>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-[#6b7280] text-sm">Customs & add New Features by</span>
                    <div class="flex items-center gap-2">
                        <img src="tangsky_avatar.jpg" class="w-6 h-6 rounded-full object-cover border border-white/10" referrerPolicy="no-referrer" onerror="this.src='https://api.dicebear.com/7.x/avataaars/svg?seed=Tangsky'" />
                        <span class="text-white font-medium text-sm">Tangsky</span>
                    </div>
                </div>
            </div>
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='TangskyMusify',cover=FI,artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'TangskyMusify',cover:FI,artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            setTimeout(function(){FullPlayer.open();loadTrack(S.ct);},400);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='TangskyMusify',cover=FI;
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','TangskyMusify',FI);});
    },
    ...
    switch(t){
        S.at=t;['home','search','library','downloads','dev'].forEach(function(id){var v=gid('view-'+id);if(v)v.style.display='none';});
        if(t==='library'){Library.render();}
        if(t==='home'){Home.render();}
        if(t==='search'){Search.onShow();}
        if(t==='downloads'){Downloads._open=true;Downloads.render();}else{Downloads._open=false;}
        var view=gid('view-'+t);if(view)view.style.display='block';
        ['home','search','library','downloads','dev'].forEach(function(n){var b=gid('nav-'+n);if(!b)return;b.classList.remove('text-[#cfd3d8]');b.classList.add('text-[#6b7280]');var i=b.querySelector('i, svg');if(i)i.classList.remove('fill-current');});
